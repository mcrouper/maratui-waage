...
### Screens (`src/screens/`)
Each screen is a zero-size struct implementing the `Board` trait (`fn render(state, area, frame)`). There is a **single physical button** (Button1), classified by `ButtonState` in `src/button.rs` into `ButtonPressType::{Short, Long, Double}` (`Double` = two Shorts released within 400ms of each other, see `DOUBLE_PRESS_WINDOW_MS`). A lone Short is *not* dispatched the instant it's released — `update()` holds it back so it can still turn into a Double, and `poll()` (called every main-loop tick, independent of press state) fires it once the window passes with no pairing second press; this means every plain Short press action has up to ~400ms of added latency, which is the trade-off for the Short and Double gestures not colliding. `Screen` (`src/screens/screen.rs`) is `{ Dashboard (default), Graphs, Debug }`; a short press toggles `Dashboard ↔ Graphs` (`Screen::next`/`previous` are the same swap — one button, no real "previous"), a long press enters/exits `Debug` (`screen_before_debug` restores whichever of the two it interrupted), a double press zeroes the scale (see Scale below, ignored while the calibration wizard is active). The boot/loading screen (`Connecting`) is not a `Screen` variant — `MaraUi::draw()` shows it whenever `GlobalAppState::machine_state.last_frame` is still `None`, before any telemetry (or, on the simulator/`EnterOfflineMode`, a synthetic offline frame) has arrived. `CalibrationWizard` is not part of the rotation either — it overlays whatever screen is active whenever `GlobalAppState::calibration_step` is `Some`.
...
- The Dashboard shows live weight (`GlobalAppState::weight_dg`, in decigrams) and auto-tares the moment a shot starts (`AppEvent::ShotStarted`) so it displays net extracted weight; it also auto-tares once on the first `WeightUpdated` after boot (`fsm.rs`'s `WeightUpdated` handler). `AppStateMachine::tare_scale` is the shared zero-out helper, also invoked by a double press (`ButtonPressType::Double`) for a manual zero at any time. The shot gauge ("%" column) tracks extracted weight, not elapsed time: `WEIGHT_GAUGE_MAX_DG` (40g) is 100%.
...
| Z | Button1 double press (zero the scale) — injected directly, since the simulator has no real button timing to double-press |
